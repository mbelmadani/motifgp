...
class MOEAD(object):
    ...
    def __init__(self, population, toolbox, mu, cxpb,
                 mutpb, logbook, ngen=0, maxEvaluations=0, timelimit=0,
                 T=20, nr=2, delta=0.9, stats=None,
                 halloffame=None, verbose=__debug__, dataDirectory="weights",
                 termination=None):
        ...
        if termination == "auto" or ((maxEvaluations == ngen) and (timelimit == ngen) and (ngen == 0)):
            #print "maxEvaluations or ngen must be greater than 0."
            #raise ValueError
            self.termination = "auto"
        else:
            if timelimit > 0:
                self.termination = "timelimit"
            elif ngen > 0:
                self.maxEvaluations = ngen * self.populationSize_
                self.termination = "generation"
            else:
    ...
    def execute(self):
        ...
        if self.timelimit > 0:
            terminator = TimeTerminator(limit=self.timelimit)
        else:
            terminator = GenerationTerminator(limit=self.maxEvaluations)
        
        logbook = tools.Logbook()
        logbook.header = ['gen', 'evals'] + (self.stats.fields if self.stats else [])
        updates = 0 # Counter to know when to print the stats
        ...
        if termination == "auto":
            ...
            terminator = MOEATerminationDetection(DEBUG=True)
            P = [list(y) for y in [x.fitness.values for x in self.paretoFront[:]]]
        ...
        if type(terminator) == type(MOEATerminationDetection()):
            raise NotImplementedError("MOEATerminationDetection for MOEAD is not supported at this time.")



        while not terminator.done(): #self.evaluations_ < self.maxEvaluations:
            ...
            for i in xrange(self.populationSize_):
                ...
                record = self.stats.compile(self.population) if self.stats is not None else {}
                logbook.record(gen=self.ngen, evals=self.evaluations_, **record )     
                if ((updates % self.ngen) == 0):

                    if termination == "auto":
                        Q = [list(y) for y in [x.fitness.values for x in self.paretoFront[:]]]
                        terminator.update(P,Q, updates)
                        P = [list(y) for y in [x.fitness.values for x in Q[:]]]
                #else:
                #    #print terminator.status(True)
                #    #print self.evaluations_, "/", self.maxEvaluations                    
                if termination != "auto":
                    terminator.update(gen=self.evaluations_) 
